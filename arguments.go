...
func argumentParsing(args []string) {
	...
	maxHostOccurence := parser.Int("m", "max-host-occurence", &argparse.Options{
		Required: false,
		Default:  -1,
		Help:     "Max number of a occurence of a given host to accept in the final seeds list.\nIf an host is parsed more than X times, new occurences of that host past that limit will be excluded.\n-1 value means no limit.",
	})
